...
fn suggest(prefix: &str, cache_path: &str) -> Result<Vec<String>, String> {
    ...
    if prefix.is_empty() || cache_path.trim().is_empty() { return Ok(vec![]); }
    let Some(end) = prefix_end(&prefix) else { return Ok(vec![]); };
    let dir = resolve_cache_dir(cache_path)?;
    let mut matches = BTreeMap::<String, String>::new();
    for (file, table) in [(DISTRIBUTION_DB_FILE, "entries"), (USER_DB_FILE, "user_entries")] {
        let path = dir.join(file);
        if !path.is_file() { continue; }
        let conn = Connection::open_with_flags(path, OpenFlags::SQLITE_OPEN_READ_ONLY)
            .map_err(|e| e.to_string())?;
        let languages = if table == "entries" {
            let mut stmt = conn.prepare("SELECT DISTINCT headword_language_code FROM entries").map_err(|e| e.to_string())?;
            let rows = stmt.query_map([], |r| r.get::<_, String>(0)).map_err(|e| e.to_string())?;
            rows.collect::<Result<Vec<_>, _>>().map_err(|e| e.to_string())?
        } else { vec![String::new()] };
        let language_filter = if table == "entries" { "AND headword_language_code = ?3" } else { "AND ?3 = ''" };
        ...
        for language in languages {
            let rows = stmt.query_map(rusqlite::params![prefix, end, language], |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?)))
                .map_err(|e| e.to_string())?;
            for row in rows {
...
mod tests {
    ...
    fn merges_sources_preserves_spelling_and_treats_wildcards_literally() {
        ...
        conn.execute_batch("CREATE TABLE user_entries (headword TEXT, normalized_headword TEXT PRIMARY KEY);
            INSERT INTO user_entries VALUES ('apple','apple'), ('app','app');").unwrap();
    ...
    fn missing_databases_are_an_empty_list() {
        let dir = tempfile::tempdir().unwrap();
        assert!(suggest("hello", dir.path().to_str().unwrap()).unwrap().is_empty());
    }
}
