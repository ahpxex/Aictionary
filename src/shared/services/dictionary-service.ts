...
function createMockDefinition(word: string): WordDefinition {
  return {
    word,
...
export async function queryDictionary(word: string): Promise<WordDefinition> {
  ...
  try {
    ...
    return createMockDefinition(trimmed);
  }
}
