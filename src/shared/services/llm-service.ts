...
- headword_summary：如果 is_meaningful 为 true，写一句概括；如果为 false，返回空字符串。

headword_summary 的写法（这是最重要的部分）：

写成一句通顺的中文，勾勒这个词的语义版图——核心义是什么，又如何引申出其他用法。不要写成“释义一；释义二；释义三”那样的罗列，那是词典释义，不是概括。长度控制在 40 字左右，最多不超过 70 字。

参考这些真实词条的写法：

- what：英语中围绕“什么、哪一个、什么样”展开的疑问词，也可引申为泛指事物、连接从句，或表示惊讶、追问和停顿。
- run：“run”最基本的意思是快速移动或奔跑，也可指奔跑这一动作；由“持续移动或运转”可引申为液体流动、机器或活动运行，以及经营、管理某事物。它还有竞选、逃跑、延续、花费等多种常见用法。
- Tuesday：Tuesday 指星期二；也可引申为平淡、普通的一天。
- doubly-linked：形容数据结构中的元素彼此双向连接：每个元素都指向前一个元素和后一个元素。
- give someone the bag：一个过时的英语习语，表示使某人失望、让某人的期待落空。
